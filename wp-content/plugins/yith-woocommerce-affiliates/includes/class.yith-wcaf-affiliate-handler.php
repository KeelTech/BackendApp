...
if (!class_exists('YITH_WCAF_Affiliate_Handler')) {
	...
	class YITH_WCAF_Affiliate_Handler
	{
		...
		public function __construct()
		{
			...
			add_action('profile_update', array($this, 'save_affiliate_extra_fields'));
					...
					if (!$affiliate) {
						...
						if ($validation_error->get_error_code()) {
							...
							if ($id) {
								...
								add_user_meta($customer_id, '_yith_wcaf_application_date', current_time('mysql'));
...
<?php
		...
		public function save_affiliate_extra_fields($user_id)
		{
			...
			if ($is_affiliate && !$affiliate) {
				 ...
				 *
				 * This is placed here, instead of directly inside ->update, since most updates of the affiliate won't require
				 * to register role again, while this procedure will always require this action to be performed
				 * On the opposite side, ->add executed ->add_role, since any addition of affiliate will also require the role
				 * to be registered for the user
				 *
				 * @since 1.7.4
				 */
				$this->add_role($affiliate['ID']);
