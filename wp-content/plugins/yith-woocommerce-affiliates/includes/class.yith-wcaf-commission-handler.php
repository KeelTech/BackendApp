...
if ( ! class_exists( 'YITH_WCAF_Commission_Handler' ) ) {
	...
	class YITH_WCAF_Commission_Handler {
		...
		public function __construct() {
			...
			$this->_status_labels_map                  = apply_filters(
				'yith_wcaf_commission_status_labels_map',
				array(
		...
		public function order_refund_handler( $new_refund_id ) {
			...
			foreach ( $order->get_refunds() as $_refund ) {
				...
				foreach ( $_refund->get_items() as $item_id => $item ) {
					...
					if ( $commission_id = wc_get_order_item_meta( $original_item_id, '_yith_wcaf_commission_id', true ) ) {
						...
						$amount             = (float) $refund_amount * (float) $commission['rate'] / 100;
			...
			foreach ( $global_refunds as $_refund ) {
				...
				foreach ( $order->get_items() as $item_id => $item ) {
					...
					if ( $commission_id ) {
						...
						$amount    = (float) abs( $to_refund * (float) $commission['rate'] / 100 ) * - 1;
			...
			foreach ( $refunds as $refund_id => $commissions_refunded ) {
				foreach ( $commissions_refunded as $commission_id => $amount ) {
					$commission = $this->get_commission( $commission_id );
					$note       = $refund_id == $new_refund_id ? sprintf( __( 'Refunded %1$s due to refund #%2$s creation', 'yith-woocommerce-affiliates' ), wc_price( abs( $amount ) ), $refund_id ) : '';
			...
			if ( $meta_key == '_refunded_commissions' ) {
				...
				if ( ! empty( $meta_value ) ) {
					foreach ( $meta_value as $commission_id => $refund ) {
						$commission = $this->get_commission( $commission_id );
						$this->change_commission_amount( $commission_id, abs( (float) $refund ), sprintf( __( 'Restored %1$s due to refund #%2$s deletion', 'yith-woocommerce-affiliates' ), wc_price( abs( $refund ) ), $object_id ) );
						$this->change_commission_refund( $commission_id, (float) $refund );
					}
				}
			}
		}
		...
		public function update( $commission_id, $args ) {
			...
			$res = $wpdb->update( $wpdb->yith_commissions, $args, array( 'ID' => $commission_id ) );
		...
		public function delete_single_commission( $commission_id, $force = false, $delete_rate = false ) {
			...
			if ( in_array( $commission['status'], $this->_assigned_status ) ) {
				YITH_WCAF_Affiliate_Handler()->update_affiliate_total( $commission['affiliate_id'], - 1 * (float) $commission['amount'] );
			}

			$res = $this->delete( $commission['ID'] );
		...
		public function restore_from_trash( $commission_id ) {
			...
			$message           = sprintf( __( '%1$s restored this item; system automatically assigned %2$s status', 'yith-woocommerce-affiliates' ), $user->user_login, $this->get_readable_status( $commission_status ) );
		...
		protected function _calculate_line_item_commission( $order, $line_item_id, $line_item, $rate ) {
			...
			$line_total = apply_filters( 'yith_wcaf_line_item_commission_total', (float) $order->$get_item_amount( $line_item, 'yes' != $this->_exclude_tax, false ) * $line_item['qty'], $order, $line_item_id, $line_item, $rate );
			...
			if ( apply_filters( 'yith_wcaf_line_total_check_amount_total', true ) && $amount >= $line_total ) {
				return $line_total;
			}
		...
		public function get_commissions( $args = array() ) {
			...
			if ( ! empty( $args['exclude'] ) ) {
				...
				$query .= ' AND yc.ID NOT IN (' . esc_sql( implode( ',', $args['exclude'] ) ) . ')';
			}
		...
		public function change_commission_amount( $commission_id, $difference, $note = '' ) {
			...
			$new_amount = $difference + (float) $commission['amount'];
			$new_amount = ( $new_amount > 0 ) ? $new_amount : 0;

			if ( in_array( $commission['status'], $this->_assigned_status ) ) {
				YITH_WCAF_Affiliate_Handler()->update_affiliate_total( $commission['affiliate_id'], (float) $difference );
			}
		...
		public function change_commission_affiliate( $commission_id, $new_affiliate_id, $note = '' ) {
			...
			YITH_WCAF_Commission_Handler()->update(
				$commission['ID'],
				array(
		...
		public function change_commission_refund( $commission_id, $difference, $note = '' ) {
			...
			YITH_WCAF_Affiliate_Handler()->update_affiliate_refunds( $commission['affiliate_id'], (float) $difference );

			$new_amount = $difference + (float) $commission['refunds'];
			$new_amount = ( $new_amount > 0 ) ? $new_amount : 0;
		...
		public function change_commission_status( $commission_id, $new_status, $note = '' ) {
			...
			if ( in_array( $old_status, $this->_unassigned_status ) && in_array( $new_status, $this->_assigned_status ) ) {
				if ( $old_status == 'refunded' ) {
					...
					$this->change_commission_refund( $commission_id, - 1 * (float) $commission['refunds'] - $this->get_total_commission_refund( $commission_id ) );
				}

				YITH_WCAF_Affiliate_Handler()->update_affiliate_total( $commission['affiliate_id'], (float) $commission['amount'] );
			} elseif ( in_array( $new_status, $this->_unassigned_status ) && in_array( $old_status, $this->_assigned_status ) ) {
				if ( $new_status == 'refunded' ) {
					$this->change_commission_amount( $commission_id, - 1 * (float) $commission['amount'] );
					$this->change_commission_refund( $commission_id, (float) $commission['amount'] );
				}
			...
			if ( ! in_array( $old_status, $this->_payment_status ) && in_array( $new_status, $this->_payment_status ) ) {
				YITH_WCAF_Affiliate_Handler()->update_affiliate_payments( $commission['affiliate_id'], (float) $commission['amount'] );
			} elseif ( ! in_array( $new_status, $this->_payment_status ) && in_array( $old_status, $this->_payment_status ) ) {
				YITH_WCAF_Affiliate_Handler()->update_affiliate_payments( $commission['affiliate_id'], - 1 * (float) $commission['amount'] );
			}
		...
		public function per_status_count( $status = 'all', $args = array() ) {
			...
			$res = empty( $args ) ? get_transient( 'yith_wcaf_commissions_per_status_count' ) : false;

			if ( empty( $res ) ) {
				$query     = "SELECT v.status, COUNT( v.status ) AS status_count FROM (
                      SELECT yc.* FROM {$wpdb->yith_commissions} AS yc
                      LEFT JOIN {$wpdb->yith_affiliates} AS ya ON ya.ID = yc.affiliate_id
                      LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta AS im ON im.order_item_id = yc.line_item_id
                      LEFT JOIN {$wpdb->posts} AS p ON p.ID = im.meta_value 
                      WHERE ( im.meta_key = '_product_id' || im.meta_key = '_variation_id' )
				...
				$query .= ' GROUP BY yc.ID ) AS v GROUP BY status';
				...
				empty( $args ) && set_transient( 'yith_wcaf_commissions_per_status_count', $res, MONTH_IN_SECONDS );
			}
		...
		/**
		 * Clear transients related to commissions
		 *
		 * @return void
		 * @since 1.7.4
		 */
		public function clear_transients() {
			delete_transient( 'yith_wcaf_commissions_per_status_count' );
		}
		...
		public function process_bulk_actions() {
			if ( ! empty( $_REQUEST['commissions'] ) ) {
				...
				$current_action = ( '-1' == $current_action && isset( $_REQUEST['action2'] ) ) ? $_REQUEST['action2'] : $current_action;
		...
		public function add_order_metabox() {
			add_meta_box(
				...
				__( 'Referral Commissions', 'yith-woocommerce-affiliates' ),
				array(
					$this,
		...
		public static function get_instance() {
			if ( class_exists( 'YITH_WCAF_Commission_Handler_Premium' ) ) {
				...
				if ( is_null( self::$instance ) ) {
					self::$instance = new YITH_WCAF_Commission_Handler();
				}
