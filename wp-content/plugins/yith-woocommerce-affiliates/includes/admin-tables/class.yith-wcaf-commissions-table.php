...
if ( ! class_exists( 'YITH_WCAF_Commissions_Table' ) ) {
	...
	class YITH_WCAF_Commissions_Table extends WP_List_Table {
		...
		public function column_amount( $item ) {
			...
			$column .= '<strong>' . wc_price( $item['amount'], method_exists( $order, 'get_currency' ) ? array(
					'currency' => $order->get_currency() ) : array()  ) . '</strong>';
		...
		public function get_columns() {
			$columns = array(
				...
				'payments'        => __( 'Payment', 'Column label for payments related to a single commission', 'yith-woocommerce-affiliates' ),
