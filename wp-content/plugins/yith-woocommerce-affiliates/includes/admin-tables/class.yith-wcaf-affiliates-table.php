...
if ( ! class_exists( 'YITH_WCAF_Affiliates_Table' ) ) {
	...
	class YITH_WCAF_Affiliates_Table extends WP_List_Table {
		...
		public function column_affiliate( $item ) {
			...
			$column .= sprintf( '%s%s<small class="meta email">%s</small><small class="meta">%s: %s</small>', get_avatar( $item['user_id'], 32 ), $username, $user_email, _x( 'Payment', 'Label for payment email for the affiliate (admin view)', 'yith-woocommerce-affiliates' ), $payment_email );
