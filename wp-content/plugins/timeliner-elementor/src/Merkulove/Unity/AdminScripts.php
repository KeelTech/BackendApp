...
final class AdminScripts {
	...
	private function settings_scripts() {
        ...
        wp_enqueue_script( 'mdp-timeliner-elementor-ui', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/merkulov-ui' . Plugin::get_suffix() . '.js', [], Plugin::get_version(), true );

        /** Prepare values to pass to JS. */
        $to_js = [
            'ajaxURL'   => admin_url('admin-ajax.php'),
            'nonce'     => wp_create_nonce( 'timeliner-elementor' ), // Nonce for security.
        ];

        wp_enqueue_script( 'mdp-timeliner-elementor-unity-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
        wp_localize_script( 'mdp-timeliner-elementor-unity-admin', 'mdpTimelinerElementor', $to_js );
        wp_enqueue_script( 'mdp-timeliner-elementor-admin', Plugin::get_url() . 'js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
