...
final class PluginUpdater {
     ...
     * @var string
	 **/
	private $opt_plugin_info = 'mdp_timeliner_elementor_plugin_info';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-timeliner-elementor/timeliner-elementor.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	private function compare_with_new( $plugin_info, $wp_version, $update_plugins ) {
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['timeliner-elementor/timeliner-elementor.php'] = (object)[
				'slug' => 'timeliner-elementor',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'timeliner-elementor' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
	        ...
	        if ( ! $plugin_info->mdp_timeliner_elementor_plugin_info ) {
		        return false;
	        } else {
		        $plugin_info = json_decode( $plugin_info->mdp_timeliner_elementor_plugin_info->body, false );
	        }
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! TabActivation::get_instance()->is_activated() ) {
			...
			esc_attr_e( 'Please visit the plugin page on the Envato market to ', 'timeliner-elementor' );
			echo ' <a href="https://1.envato.market/timeliner-elementor" target="_blank">';
			esc_attr_e( 'download ', 'timeliner-elementor' );
			echo '</a> ';
			esc_attr_e( 'the latest version.', 'timeliner-elementor' );
		}
