...
/**
 * Class used to implement plugin update mechanism.
 **/
final class PluginUpdater {
	 ...
	 **/
	private $opt_plugin_info = 'mdp_lottier_elementor_plugin_info';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-lottier-elementor/lottier-elementor.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	private function compare_with_new( $plugin_info, $wp_version, $update_plugins ) {
		...
		$plugin = LottierElementor::get_instance();
		$current_version = LottierElementor::$version;
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['lottier-elementor/lottier-elementor.php'] = (object)[
				'slug' => 'lottier-elementor',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'lottier-elementor' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
            ...
            $plugin_info = json_decode( $plugin_info->mdp_lottier_elementor_plugin_info->body, false );
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! PluginActivation::get_instance()->is_activated() ) {
			...
			esc_attr_e( 'Please visit the plugin page on the Envato market to ', 'lottier-elementor' );
			echo ' <a href="https://1.envato.market/lottier-elementor" target="_blank">';
			esc_attr_e( 'download ', 'lottier-elementor' );
			echo '</a> ';
			esc_attr_e( 'the latest version.', 'lottier-elementor' );
		}
