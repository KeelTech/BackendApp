...
final class ReporterServer {
	...
	public function get_title() {
		...
		return esc_html__( 'Server Environment', 'lottier-elementor' );
	...
	public function get_fields() {
        ...
        $checks = [];
        $checks = $this->add_check( $checks, $server_checks, 'os', esc_html__( 'Operating System', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'software', esc_html__( 'Software', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'mysql_version', esc_html__( 'MySQL version', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'php_version', esc_html__( 'PHP Version', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'write_permissions', esc_html__( 'Write Permissions', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'zip_installed', esc_html__( 'ZIP Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'curl_installed', esc_html__( 'cURL Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'elementor_installed', esc_html__( 'Elementor Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'wpbakery_installed', esc_html__( 'WPBakery Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'allow_url_fopen', esc_html__( 'allow_url_fopen', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'dom_installed', esc_html__( 'DOM Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'xml_installed', esc_html__( 'XML Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'bcmath_installed', esc_html__( 'BCMath Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'mbstring_installed', esc_html__( 'mbstring Installed', 'lottier-elementor' ) );
        $checks = $this->add_check( $checks, $server_checks, 'server_time', esc_html__( 'Server Time Sync', 'lottier-elementor' ) );
    ...
    public function get_wpbakery_installed() {
        ...
        return [
            'value' => $wpbakery_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-elementor') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-elementor' ),
            'warning' => ! $wpbakery_installed,
            'recommendation' => esc_html__('You need install and activate WPBakery builder. Go to WPBakery site (wpbakery.com) for details.', 'lottier-elementor' )
        ];
    ...
    public function get_mbstring_installed() {
        ...
        return [
            'value' => $mbstring_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-elementor' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-elementor' ),
            'warning' => ! $mbstring_installed,
            'recommendation' => esc_html__('You must enable mbstring extension (Multibyte String) in PHP. Contact the support service of your hosting provider. They know what to do.', 'lottier-elementor' )
        ];
    }
    ...
    public function get_server_time() {
        ...
        if ( $curl_errno ) {
            $time_ok = false;
            return [
                'value' => $time_ok ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-elementor') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-elementor' ),
                'warning' => ! $time_ok,
                'recommendation' => esc_html__('Failed to check time synchronization on your server. Your server\'s clock must be in sync with network time protocol - NTP.', 'lottier-elementor' )
            ];
        }
        ...
        return [
            'value' => $time_ok ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-elementor') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-elementor' ) . '<br>Google Time: ' . $google_time . '<br>&nbsp;&nbsp;&nbsp;Local Time: ' . $server_time,
            'warning' => ! $time_ok,
            'recommendation' => esc_html__( ' Your server\'s clock is not in sync with network time protocol - NTP. Contact the support service of your hosting provider. They know what to do.', 'lottier-elementor' )
        ];
