...
from keel.api.v1.document.serializers import (
...
from .helpers.email_helper import EmailTemplateHelper
...
class UserViewset(GenericViewSet):
    ...
    def signup(self, request, format="json"):
        ...
        validated_data[
            "user_type"
        ] = user_model.CUSTOMER  # add user type to validated data
...
class GeneratePasswordReset(GenericViewSet):
    ...
    def token(self, request):
        ...
        try:
            ...
            obj, created = PasswordResetToken.objects.get_or_create(
                user=user, reset_token=current_time, expiry_date=expiry_date
            )
            context = {"token": current_time}
    ...
    def facebook(auth_token):
        ...
        try:
            ...
            profile = graph.get_object(
                id="me", fields="first_name, last_name, id, email"
            )
            return profile
...
class ProfileView(GenericViewSet):
    ...
    serializer_class_education_assessment = (
        serializers.EducationalCreationalAssessmentLabelSerializer
    )
    ...
    def get_queryset_qualification(self, request):
        ...
        if queryset:
            serializer = self.serializer_class_qualification(
                queryset, many=True, context={"labels": labels}
            )
    ...
    def get_queryset_education_assessment(self, request):
        get_labels = EducationalCreationalAssessmentLabel.objects.filter(
            user_label="user"
        )
        labels = {}
        for label in get_labels:
            ...
            labels[
                "canadian_equivalency_summary_label"
            ] = label.canadian_equivalency_summary_label
        queryset = EducationalCreationalAssessment.objects.filter(user=request.user)
        if queryset:
            serializer = self.serializer_class_education_assessment(
                queryset, many=True, context={"labels": labels}
            )
            return serializer.data
        else:
    ...
    def get_queryset_relative_in_canada(self, request):
        ...
        if queryset:
            serializer = self.serializer_class_relative_in_canada(
                queryset, context={"labels": labels}
            )
            return serializer.data
        else:
    ...
    def get_queryset_experience(self, request):
        ...
        if queryset:
            serializer = self.serializer_class_experience(
                queryset, many=True, context={"labels": labels}
            )
            for label in serializer.data:
                label.pop("labels")
    ...
    def get_queryset_profile(self, request):
        ...
        if profile:
            serializer = self.serializer_class_profile(
    ...
    def get_queryset_cases(self, user):
        get_case = None
        try:
            get_case = Case.objects.select_related("plan").get(
    ...
    def create_full_profile(self, request):
        ...
        create_education_assessment = (
            EducationalCreationalAssessmentView.educational_creational_assessment(
                request
            )
        )
        ...
        response["message"] = {
            ...
            "qualification": create_qualification.data,
            "work_experience": create_work_experience.data,
            "relative_in_canada": create_relative_in_canada.data,
    ...
    def update_full_profile(self, request):
        ...
        update_relative_in_canada = RelativeInCanadaView.update_relative_in_canada(
            request
        )
        # update education instance
        update_education_assessment = EducationalCreationalAssessmentView.update_educational_creational_assessment(
            request
        )
...
class QualificationView(GenericViewSet):
    ...
    def extract(datas):
        ...
        for info in datas:
            ...
            try:
                ...
                logger.error(
                    "ERROR: AUTHENTICATION:QualificationView:extract " + str(e)
                )
                pass
    ...
    def qualification(self, request, **kwargs):
        ...
        try:
            ...
            logger.error(
                "ERROR: AUTHENTICATION:QualificationView:qualification " + str(e)
            )
            response["message"] = str(e)
            response["status"] = 0
            return Response(response, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            logger.error(
                "ERROR: AUTHENTICATION:CreateQualificationView:qualification " + str(e)
            )
            response["message"] = str(e)
            response["status"] = 0
            return Response(response, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def create(validated_data):
        ...
        try:
            qualification, created = CustomerQualifications.objects.update_or_create(
                id=id,
                defaults={
                    "institute": institute,
                    "degree": degree,
            ...
            logger.error(
                logging_format(LOGGER_LOW_SEVERITY, "QualificationView:create"),
                "",
                description=str(err),
            )
    ...
    def update_qualification(self, request):
        ...
        try:
            ...
            logger.error(
                "ERROR: AUTHENTICATION:QualificationView:update_qualification " + str(e)
            )
            response["message"] = str(e)
            response["status"] = 0
            return Response(response, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class WorkExperienceView(GenericViewSet):
    ...
    def create(validated_data):
        ...
        try:
            work, created = CustomerWorkExperience.objects.update_or_create(
                id=id,
                defaults={
                    "job_type": job_type,
            ...
            logger.error(
                logging_format(LOGGER_LOW_SEVERITY, "WorkExperienceView:create"),
                "",
                description=str(err),
            )
            raise serializers.ValidationError(
                "Customer Work experience with ID does not exist"
            )
        return work
...
class EducationalCreationalAssessmentView(GenericViewSet):
    ...
    serializer_class_update = (
        serializers.EducationalCreationalAssessmentUpdateSerializer
    )
    ...
    def extract(datas):
        ...
        for info in datas:
            customer_work_info = {
                ...
                "canadian_equivalency_summary": info[
    ...
    def educational_creational_assessment(self, request):
        ...
        try:
            ...
            logger.error(
                "ERROR: AUTHENTICATION:EducationalCreationalAssessment " + str(e)
            )
            response["message"] = str(e)
            response["status"] = 0
            return Response(response, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def create(validated_data):
        ...
        canadian_equivalency_summary = validated_data.get(
            "canadian_equivalency_summary"
        )
        user = validated_data.get("user")
        try:
            (
                ...
                created,
            ) = EducationalCreationalAssessment.objects.update_or_create(
                id=id,
                defaults={
            ...
            logger.error(
                logging_format(
                    LOGGER_LOW_SEVERITY, "EducationalCreationalAssessmentView:create"
                ),
                "",
                description=str(err),
            )
            raise serializers.ValidationError(
                "Education Assessment with ID does not exist"
            )
...
class LoginOTP(GenericViewSet):
    ...
    def generate(self, request, format=None):
        ...
        if err:
            log_error(
                "CRITICAL", "LoginOTP.generate", "Failed to send SMS", err=str(err)
            )
            response["status"] = 1
...
class UploadDocument(GenericViewSet):
    ...
    def upload(self, request, format="json"):
        ...
        response = {"status": 0, "message": "File Uploaded successfully", "data": ""}
        resp_status = status.HTTP_200_OK
        ...
        try:
            ...
            log_error(
                "ERROR",
                "UploadDocument:upload DocumentInvalid",
                str(user_id),
                err=str(e),
            )
            response["status"] = 1
            ...
            log_error(
                "ERROR", "UploadDocument:upload Exception", str(user_id), err=str(e)
            )
            response["status"] = 1
        ...
        try:
            ...
            if task_id:
                task_serializer = serializers.TaskIDSerializer(
            ...
            log_error(
                "ERROR",
                "UploadDocument: upload taskValidation",
                str(user_id),
                err=str(e),
            )
            response["status"] = 1
    ...
    def fetch(self, request, format="json"):
        ...
        try:
            user_docs = (
                UserDocument.objects.select_related("doc", "doc__doc_type")
                .filter(user_id=actual_user_id, deleted_at__isnull=True)
                .order_by("-updated_at")
            )
            user_doc_serializer = serializers.ListUserDocumentSerializer(
                user_docs, many=True
            )
            ...
            log_error(
                "ERROR", "UploadDocument:fetch exception", str(user.id), err=str(e)
            )
            response["status"] = 1
            response["message"] = GENERIC_ERROR
            resp_status = status.HTTP_500_INTERNAL_SERVER_ERROR
    ...
    def deleteUserDoc(self, request, format="json", **kwargs):
        ...
        response = {"status": 0, "message": "User Document is deleted", "data": ""}
        ...
        try:
            ...
            log_error(
                "ERROR",
                "UploadDocument: deleteUserDoc ",
                str(user_id),
                err="Invalid User Doc Id",
            )
            response["status"] = 1
            ...
            log_error(
                "ERROR", "UploadDocument: deleteUserDoc ", str(user_id), err=str(e)
            )
            response["status"] = 1
            response["message"] = GENERIC_ERROR
            resp_status = status.HTTP_500_INTERNAL_SERVER_ERROR
        ...
        if user_doc.doc.owner_id != str(user_id):
            log_error(
                ...
                "UploadDocument: deleteUserDoc ",
                str(user_id),
                err="LoggedIn User is not a owner of document",
            )
            response["status"] = 1
        ...
        try:
            ...
            log_error(
                "ERROR",
                "UploadDocument: deleteUserDoc mark_delete",
                str(user_id),
                err=str(e),
            )
            response["status"] = 1
...
class AgentView(GenericViewSet):
    ...
    def agent_profile(self, request):
        ...
        response["message"] = {
            "agent_details": user_serializer,
            "agent_profile": serializer,
        }
        return Response(response)
