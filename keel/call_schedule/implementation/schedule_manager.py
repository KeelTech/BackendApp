...
class CallScheduleManager(ICallScheduleManager):
    ...
    def _get_host_user_obj(self):
        return self._user_case_model_obj.agent if self._user_case_model_obj else User.objects.filter(user_type=User.ACCOUNT_MANAGER).first()  # TODO: return default user
